...
typedef struct edict_oblivion_ext_s {
  ...
  struct edict_s
      *script_target;        // auxiliary entity referenced by scripted actions
...
typedef struct gitem_s {
  ...
  char
      *precaches; // string of all models, sounds, and images this item will use
} gitem_t;
...
#define DAMAGE_NO_PROTECTION                                                   \
  0x00000020 // armor, shields, invulnerability, and godmode have no effect
...
void fire_blaster_with_mod(edict_t *self, vec3_t start, vec3_t aimdir,
...
struct edict_s {
  ...
  float touch_debounce_time; // are all these legit?  do we need more/less of
                             // them?
