...
void M_WorldEffects(edict_t *ent) {
  ...
  if (ent->waterlevel == 0) {
    if (ent->flags & FL_INWATER) {
      gi.sound(ent, CHAN_BODY, gi.soundindex("player/watr_out.wav"), 1,
               ATTN_NORM, 0);
  ...
  if (!(ent->flags & FL_INWATER)) {
    if (!(ent->svflags & SVF_DEADMONSTER)) {
      if (ent->watertype & CONTENTS_LAVA)
        if (random() <= 0.5)
          gi.sound(ent, CHAN_BODY, gi.soundindex("player/lava1.wav"), 1,
                   ATTN_NORM, 0);
        else
          gi.sound(ent, CHAN_BODY, gi.soundindex("player/lava2.wav"), 1,
                   ATTN_NORM, 0);
      else if (ent->watertype & CONTENTS_SLIME)
        gi.sound(ent, CHAN_BODY, gi.soundindex("player/watr_in.wav"), 1,
                 ATTN_NORM, 0);
...
void monster_triggered_spawn_use(edict_t *self, edict_t *other,
                                 edict_t *activator) {
...
void monster_start_go(edict_t *self) {
  ...
  if (self->combattarget) {
    ...
    target = NULL;
    while ((target = G_Find(target, FOFS(targetname), self->combattarget)) !=
           NULL) {
      if (strcmp(target->classname, "point_combat") != 0) {
        gi.dprintf(
...
void deatom_think(edict_t *self) {
	...
	if (!self->deatom_target_ent || !self->deatom_target_ent->client)
	{
		self->deatom_target_ent = NULL;
		self->think = G_FreeEdict;
		self->nextthink = level.time + 60.0f;
		return;
	}

	VectorCopy(self->deatom_target_ent->s.origin, target_dir);
	target_dir[2] += self->deatom_target_ent->viewheight;
...
=================
*/
void deatom_touch(edict_t *self, edict_t *other, cplane_t *plane,
                  csurface_t *surf) {
	vec3_t	normal;
	...
	if (other->takedamage)
	{
		...
		T_Damage(other, self, self->owner, self->velocity, self->s.origin, normal,
			self->dmg, 1, DAMAGE_ENERGY, MOD_DEATOMIZER);
	}
...
void fire_deatom(edict_t *self, vec3_t start, vec3_t dir, int damage,
	...
	deatom->deatom_target_ent = best_ent;
	...
	if (tr.fraction < 1.0f)
	{
		VectorMA(deatom->s.origin, -10.0f, dir, deatom->s.origin);
		if (tr.ent == self)
			G_FreeEdict(deatom);
		else
