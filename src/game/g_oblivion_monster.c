...
void deatom_touch(edict_t *self, edict_t *other, cplane_t *plane,
  ...
  gi.sound(self, CHAN_WEAPON, gi.soundindex("deatom/dimpact.wav"), 1, ATTN_NORM,
           0);

  if (other->takedamage) {
    // Note: trace_t info isn't fully passed in generic touch functions in some
    // Q2 bases, but G_RunEntity usually calls touch with (ent, other, plane,
    // surf). We can construct a trace_t or just pass plane->normal if valid.
...
void fire_deatom(edict_t *self, vec3_t start, vec3_t dir, int damage,
  ...
  gi.sound(self, CHAN_WEAPON, gi.soundindex("deatom/dfire.wav"), 1, ATTN_NORM,
           0);
